...
| `lint` | `false` | Opt-in. When true, runs `swift-format lint --recursive --strict --parallel --follow-symlinks` (via `xcrun swift-format`, bundled in the Xcode/Swift 6 toolchain — no install step) over top-level directories (recursed, `.swift`-filtered, symlinks followed) plus top-level `*.swift` files, excluding `.build` and `.git`. Applies in both `spm` and `xcodegen` mode. **Runs AFTER toolchain selection** ("Setup Swift CI (spm)" / "Select Xcode (xcodegen)"), deliberately not before: `xcrun` resolves against whatever `xcode-select` currently points at, so linting before selection would check against the runner image's default Xcode rather than the caller's pinned `xcode-version` for every hosted caller — the ordering, not toolchain-bundling alone, is what keeps the result stable across GitHub's periodic image-default rotations and comparable between hosted and self-hosted callers. Fails loudly (`::error`, exit 1) if zero `*.swift` files are found in scope, rather than reporting a silent green — a lint that checked nothing is not the same as a lint that found nothing wrong. Non-`.swift` top-level files (`README.md`, `Package.resolved`, JSON configs, …) are deliberately NOT passed as explicit arguments — `swift-format lint` only filters to `*.swift` when recursing into a directory; an explicit file argument is parsed as Swift regardless of extension, so passing e.g. `Package.resolved` directly floods the run with parse errors on JSON syntax. **Check only — never rewrites.** A repo with unformatted history will fail immediately on first enable; run a one-time `swift-format format --recursive --in-place <paths>` pass and land it *before* flipping this on, as its own PR (do not bundle a mechanical reformat into the PR that enables the gate). Default `false` is a no-op for every existing caller. |
...
Verify locally first with `xcrun swift-format lint --recursive --strict --parallel --follow-symlinks <top-level dirs and *.swift files, excluding .build/.git>` (same invocation CI runs, after selecting the same Xcode the caller pins — see the `lint` input row above for why both the ordering and the non-`.swift`-top-level-file exclusion matter) — a repo with unformatted history will fail on the first PR after enabling. Run `xcrun swift-format format --recursive --in-place <paths>` as a one-time, standalone PR before enabling, not bundled with the flag flip.
